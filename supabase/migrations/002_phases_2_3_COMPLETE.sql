-- Complete Migration for Phases 2-3: Social connections + AI generation
...
DROP POLICY IF EXISTS "Users can create own brand settings" ON brand_settings;
DROP POLICY IF EXISTS "Users can update own brand settings" ON brand_settings;
DROP POLICY IF EXISTS "Users can delete own brand settings" ON brand_settings;
...
-- Drop existing policies if they exist (to avoid conflicts)
DROP POLICY IF EXISTS "Users can view own post variants" ON post_variants;
DROP POLICY IF EXISTS "Users can create own post variants" ON post_variants;
DROP POLICY IF EXISTS "Users can update own post variants" ON post_variants;
DROP POLICY IF EXISTS "Users can delete own post variants" ON post_variants;
...
-- Drop existing policies if they exist (to avoid conflicts)
DROP POLICY IF EXISTS "Users can view own queue items" ON posting_queue;
DROP POLICY IF EXISTS "Users can create own queue items" ON posting_queue;
DROP POLICY IF EXISTS "Users can update own queue items" ON posting_queue;
...
CREATE INDEX IF NOT EXISTS brand_settings_user_id_idx ON brand_settings(user_id);
